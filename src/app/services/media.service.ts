...
export class MediaService {
  ...
  private readonly apiKey: string = 'e65c4db5bae2b9b0565c97b1e317145e'
  ...
  searchMedia(searchString: string, page: number = 1): Observable<ITmdbResponse> {          // Retorna un observable de ITmdbResponse.
    ...
    return this.httpClient.get<ITmdbResponse>(`https://api.themoviedb.org/3/search/multi?api_key=${this.apiKey}&query=${searchString}&page=${page}&include_adult=false`);
  ...
  getDetails(mediaId: number, mediaType: string): Observable<Media> {
    ...
    return this.httpClient.get<Media>(`https://api.themoviedb.org/3/${mediaType}/${mediaId}?api_key=${this.apiKey}`)

  }

  getVideo(mediaId: number, mediaType: string): Observable<ITmdbResponse> {

    return this.httpClient.get<ITmdbResponse>(`https://api.themoviedb.org/3/${mediaType}/${mediaId}/videos?api_key=${this.apiKey}&language=en-US`)
